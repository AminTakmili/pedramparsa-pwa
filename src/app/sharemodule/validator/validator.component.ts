...
export class ValidatorComponent implements OnInit {
	...
	ngOnInit() {
		this.config = {
			...
			pattern : `${this.controlName}   باید عددی که با 09 شروع شود باشد   .`
		};
	}
				...
				for (const error in errors) {
					switch (error) {
						...
						case 'maxlength':
							...
							break;
						// case 'pattern':
						// 	this.param = this.control.errors['pattern'].requiredPattern.toString();
						// 	break;
						default:
							this.param = '';
							break;
					...
					if (!this.control.dirty) {
						...
						this.config['required']=`${this.controlName} ضروری است.`
					}
